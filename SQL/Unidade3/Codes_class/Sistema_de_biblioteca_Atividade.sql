...
-- 1) Liste os nomes e cidades das editoras
SELECT nome, cidade FROM EDITORA;

-- 2) Liste os livros publicados pela editora de ID 1
SELECT titulo FROM LIVRO
WHERE id_editora = 1;

-- 3)  Insira um novo usuário chamado "Carlos Lima".
INSERT INTO USUARIO (nome, email)
VALUES ('Carlos Lima', 'carlos.lima@email.com');

SELECT * FROM USUARIO;

-- 4) Atualize a cidade da editora de ID 2.
UPDATE EDITORA
SET cidade = 'São Paulo'
WHERE id_editora = 2;

-- 5)  Remova a categoria com id 2
-- Obs: deve remover as associações com essa categora nas inserções. 
DELETE FROM CATEGORIA
WHERE id_categoria = 2;

-- 6) Liste os títulos dos livros e o nome da editora correspondente.
SELECT LIVRO.titulo, EDITORA.nome AS nome_editora
FROM LIVRO
INNER JOIN EDITORA ON LIVRO.id_editora = EDITORA.id_editora;

-- 7)  Liste os livros emprestados com nome do usuário e data.
...
-- 8) Liste os exemplares ainda não devolvidos.
...
-- 9) Adicione a coluna telefone na tabela USUARIO.
ALTER TABLE USUARIO
ADD telefone VARCHAR(15);


-- 10) Altere o tamanho do campo isbn para 13 caracteres.
ALTER TABLE LIVRO
MODIFY isbn VARCHAR(13);

-- 11) Liste autores cujo nome começa com “M” (Dica: Cláusula LIKE).
SELECT * FROM AUTOR
WHERE nome LIKE 'M%';

-- 12) . Liste autores e os livros que escreveram.
SELECT 
    AUTOR.nome, 
    LIVRO.titulo
FROM  AUTOR
...
-- 13) Exclua a tabela de empréstimos
DROP TABLE IF EXISTS EMPRESTIMO;


-- 14) Liste os empréstimos feitos entre 1º e 10 de junho de 2025.
SELECT * FROM EMPRESTIMO
WHERE data_emprestimo BETWEEN '2025-06-01' AND '2025-06-10';

-- 15)  Liste todos os usuários ordenados pelo nome
SELECT * FROM USUARIO ORDER BY nome ASC;

-- 16) Liste todos os livros, mesmo os que ainda não possuem exemplares cadastrados.
SELECT LIVRO.titulo, EXEMPLAR.codigo_de_barras
FROM LIVRO
LEFT JOIN EXEMPLAR ON LIVRO.id_livro = EXEMPLAR.id_livro;

-- 17)  Liste todos os autores, mesmo os que ainda não escreveram livros.
...
-- 18)  Liste todos os usuários e os dados de seus empréstimos, mesmo os que nunca pegaram livros emprestados
...
-- 19) Liste todos os exemplares, mesmo os que nunca foram emprestados
...
-- 20) Liste todos os empréstimos e os usuários correspondentes, incluindo empréstimos com usuários 
-- inexistentes (teste de integridade – só deve retornar NULLs se houver falhas ou dados de teste).
-- Usa RIGHT JOIN para forçar a exibição de todos os empréstimos.

SELECT EMPRESTIMO.id_emprestimo, USUARIO.nome, EMPRESTIMO.data_emprestimo, EMPRESTIMO.data_devolucao_prevista, EMPRESTIMO.data_devolucao_real, EMPRESTIMO.stats
FROM USUARIO
